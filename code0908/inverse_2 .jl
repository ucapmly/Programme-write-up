...
data1=readlines("datai1.txt")
data2=readlines("datai2.txt")
data3=readlines("datai3.txt")
...
## sensor data
sen_data=readlines("sen_datai.txt")
...
## free surface data
free_data=readlines("free_datai.txt")
...
chain_a = Lux.Chain(Dense(input_,n,Lux.σ),Dense(n,n,Lux.σ),Dense(n,n,Lux.σ),Dense(n,1))    

# additional_loss: sensor, free surface, snapshot.                                                                                     

function additional_loss(phi, θ , p)
    return 10*sum(abs2, phi[1](var_snapshot_t1, θ.depvar.u).-u_1)/length(u_1)+10*sum(abs2, phi[1](var_snapshot_t2, θ.depvar.u).-u_2)/length(u_2)+10*sum(abs2, phi[1](var_snapshot_t3, θ.depvar.u).-u_3)/length(u_3)+sum(abs2, phi[1](var_sen, θ.depvar.u).-u_sen)/length(u_sen)+sum(abs2, phi[1](var_free, θ.depvar.u).-u_free)/length(u_free)
end
...
### Misfit
p1=plot(xs, zs, u_1-u_pr_11, linetype=:contourf,title = "t=1") 
savefig("Mis1.png")       
p2=plot(xs, zs, u_2-u_pr_22, linetype=:contourf,title = "t=10") 
savefig("Mis2.png")       
p3=plot(xs, zs, u_3-u_pr_33, linetype=:contourf,title = "t=200") 
savefig("Mis3.png") 