...
function additional_loss(phi, θ , p)
    return 10*sum(abs2, phi[1](var_snapshot_t1, θ.depvar.u).-u_3_1)/length(u_3_1)+10*sum(abs2, phi[1](var_snapshot_t2, θ.depvar.u).-u_3_2)/length(u_3_2)+10*sum(abs2, phi[1](var_snapshot_t3, θ.depvar.u).-u_3_3)/length(u_3_3)+sum(abs2, phi[1](var_sen, θ.depvar.u).-u_sen)/length(u_sen)+sum(abs2, phi[1](var_free, θ.depvar.u).-u_free)/length(u_free)
end
